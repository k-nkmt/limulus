...
class _DeferredExpressionScope(dict[str, Any]):
    def __init__(
        self,
        *,
        row: Mapping[str, Any],
        ...
        resolve_array_variable: Callable[[str, Any, Mapping[str, tuple[str, ...]]], tuple[str, Diagnostic | None]],
        resolve_row_value: Callable[[Mapping[str, Any], str], Any],
    ) -> None:
        super().__init__()
        self._row = row
        self._context = context
        self._array_defs = array_defs
        self._include_row_aliases = include_row_aliases
        self._resolve_array_variable = resolve_array_variable
        ...
        self["__dim__"] = self._dim
        self["__vname__"] = self._vname
        self["__arr_get__"] = self._arr_get
        self["__var__"] = self._var

    def _dim(self, array_name: str) -> int:
        return len(self._array_defs.get(array_name, ()))

    def _vname(self, array_name: str, index_value: Any) -> str:
        variable_name, error = self._resolve_array_variable(array_name, index_value, self._array_defs)
        if error is not None:
            raise ValueError(error.message)
        return variable_name

    def _arr_get(self, array_name: str, index_value: Any) -> Any:
        variable_name, error = self._resolve_array_variable(array_name, index_value, self._array_defs)
        if error is not None:
            raise ValueError(error.message)
        return self._resolve_row_value(self._row, variable_name)

    def _var(self, key: str) -> Any:
        if hasattr(self._context, "source_slot_value"):
...
class ExpressionEvaluator:
    ...
    def _build_scope(
        ...
        *,
        ...
        return _DeferredExpressionScope(
            row=row,
            context=context,
            array_defs=array_defs,
            include_row_aliases=include_row_aliases,
            eval_scope_provider=self._eval_scope_provider,
            resolve_array_variable=self.resolve_array_variable,
